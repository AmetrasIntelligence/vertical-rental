    ...
    def _check_rental_availability(self):
        ...
        if compare_qty == -1:
            res["warning"] = {
                ...
                "message": _(
                    "You want to rent {qty_1} {uom_1} but you only "
                    "have {qty_2} {uom_2} currently available on the "
                    'stock location "{location_1}"! Make sure that you '
                    "get some units back in the meantime or "
                    're-supply the stock location "{location_2}".'
                ).format(
                    qty_1=self.rental_qty,
                    uom_1=self.product_id.rented_product_id.uom_id.name,
                    qty_2=in_location_available_qty,
                    uom_2=self.product_id.rented_product_id.uom_id.name,
                    location_1=rental_in_location.name,
                    location_2=rental_in_location.name,
                ),
            }
        return res
        ...
        for line in self:
                ...
                if line.rental_qty != line.extension_rental_id.rental_qty:
                    raise ValidationError(
                        _(
                            ...
                            "quantity ({qty}) that is different from the quantity "
                            "of the original rental ({e_qty}). This is not supported."
                        ).format(
                            name=line.product_id.name,
                            qty=line.rental_qty,
                            e_qty=line.extension_rental_id.rental_qty,
                        )
                    )
            if line.rental_type in ("new_rental", "rental_extension"):
                ...
                if line.product_uom_qty != line.sell_rental_id.rental_qty:
                    raise ValidationError(
                        _(
                            "On the sale order line with product {name} "
                            "you are trying to sell a rented product with a "
                            "quantity ({uom}) that is different from the rented "
                            "quantity ({qty}). This is not supported."
                        ).format(
                            name=line.product_id.name,
                            uom=line.product_uom_qty,
                            qty=line.sell_rental_id.rental_qty,
                        )
                    )
