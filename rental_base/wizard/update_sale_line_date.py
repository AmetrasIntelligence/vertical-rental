    ...
    def action_confirm(self):
        ...
        if self.all_line:
            if self.date_in_line:
                for line in self.line_ids:
                    message_body += _(
                        "<li>{product}: {start} - {end} -> "
                        "{date_start} - {date_end}</li>"
                    ).format(
                        product=line.order_line_id.product_id.name,
                        start=line.order_line_id.start_date,
                        end=line.order_line_id.end_date,
                ...
                message_body += _(
                    "<li>(All lines): {start} - {end} -> {date_start} - {date_end}</li>"
                ).format(
                    start=self.order_id.default_start_date,
                    end=self.order_id.default_end_date,
